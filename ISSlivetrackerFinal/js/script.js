...
function drawFadingLine(data) {
  ...
  opacity = 1;
  opacityScale = 1 / timesteps;  // Get evenly fading line
  for (let i = 0; i < data.length - 1; i++) {
    ...
    if (Math.abs(data[i][0] - data[i + 1][0]) > 10 || Math.abs(data[i][1] - data[i + 1][1]) > 10) {} else if (i > timesteps) {
      return;
    } else {
...
function loadISSpasstimes() {
  if (textInput.value == "") {  // Return if user submit is empty
    return;
  }
  if (geocodeLayer.getLayers().length == 1) {  // Clear previous geocode-markers
    geocodeLayer.clearLayers();
  }
  loadGeocode(function (response) {
    // Passing api url with correct latitude and longitude to ajax requst
    if (response[0] == 0.0 && response[1] == 0.0) {  // If geocode returns lat:0.0 lng:0.0, given location is invalid
    ...
    map.panTo(response, (animate = true));
  ...
  textInput.value = ""; // Empties input after user submit
}
